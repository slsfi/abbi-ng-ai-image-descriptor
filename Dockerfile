...
# 2. Create final image, starting from nginx:alpine,
# https://hub.docker.com/_/nginx.
FROM nginx:1.26.0 AS final
# Copy the dist folder from the build image to the final, runtime image.
COPY --from=build /abbi-ng-ai-image-descriptor/dist /usr/share/nginx/html
