...
describe('isTemperatureSupportedForModel', () => {
    ...
    it('requires OpenAI reasoning effort none when the flag is false', () => {
        ...
        expect(isTemperatureSupportedForModel(settings.model, 'none', settings.thinkingLevel)).toBe(true);
    });
    ...
    it('requires Google thinking level to be undefined when the flag is false', () => {
        ...
        expect(isTemperatureSupportedForModel(settings.model, settings.reasoningEffort, settings.thinkingLevel)).toBe(false);
        expect(isTemperatureSupportedForModel(settings.model, settings.reasoningEffort, null)).toBe(false);
        expect(isTemperatureSupportedForModel({
            ...settings.model,
            parameters: {
                ...settings.model.parameters,
                thinkingLevel: undefined,
            },
        }, settings.reasoningEffort, null)).toBe(true);
    });
});
