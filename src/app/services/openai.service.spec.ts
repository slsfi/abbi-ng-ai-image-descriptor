...
describe('OpenAiService', () => {
    ...
    it('uploads OpenAI Files API images with a 48 hour expiry and sends file ids to the model', async () => {
        const service = new OpenAiService();
        const uploadSpy = vi.fn().mockResolvedValue({ id: 'file-123' });
        const createSpy = vi.fn().mockResolvedValue({
            output_text: 'ok',
            usage: { input_tokens: 2, output_tokens: 1 },
        });
    ...
    it('reuses a cached OpenAI file upload when the file still exists', async () => {
        const service = new OpenAiService();
        const retrieveSpy = vi.fn().mockResolvedValue({ id: 'file-existing' });
    ...
    it('deletes a cached OpenAI upload and clears the image cache fields', async () => {
        const service = new OpenAiService();
        const deleteSpy = vi.fn().mockResolvedValue({ deleted: true });
    ...
    it('returns a cancelled result when an OpenAI Files API batch request is aborted', async () => {
        ...
        (service as any).client = {
            files: {
                create: vi.fn().mockResolvedValue({ id: 'file-123' }),
            },
            responses: {
        ...
        const promise = service.describeImagesWithFilesApi(createSettings({ taskType: 'transcriptionBatchTei' }), 'Prompt', [createImage()], { signal: ctrl.signal });
